...
#Caricamento immagine superiore
for i,data in pairs(themes_table) do
    if System.doesFileExist(System.currentDirectory()..data.name.."/upper.png") then
        table.insert(upper_table,Screen.loadImage(System.currentDirectory()..data.name.."/upper.png"))
    else
        table.insert(upper_table,no_preview)
    end
end
if #upper_table > 0 then
    upper_file = upper_table[1]

for i,data in pairs(themes_table) do
    if System.doesFileExist(System.currentDirectory()..data.name.."/lower.png") then
        table.insert(lower_table,Screen.loadImage(System.currentDirectory()..data.name.."/lower.png"))
    else
        table.insert(lower_table,no_preview)
    end
end
if #lower_table > 0 then
    lower_file = lower_table[1]
...
if Controls.check(pad,KEY_Y) then
    ...
    Screen.drawImage(0,0,upper_file,TOP_SCREEN)
    Screen.drawImage(0,0,lower_file,BOTTOM_SCREEN)
